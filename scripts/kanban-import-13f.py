#!/usr/bin/env python3
"""
Import the Automated 13F Holdings project and tasks directly to the kanban board API.
Requires the kanban app running (http://localhost:3004 UI, http://localhost:3005 API).

Usage:
  python scripts/kanban-import-13f.py
  python scripts/kanban-import-13f.py --api-url http://localhost:3005
"""
...
IMPORT_CSV = WORKSPACE / "outputs" / "kanban-13f-project-import.csv"
DEFAULT_API = "http://localhost:3005"


def main() -> int:
    ap = argparse.ArgumentParser(description="Import 13F project to kanban board via API.")
    ...
    with open(IMPORT_CSV, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            column = (row.get("Column") or "done").strip().strip('"').lower().replace(" ", "-")
