...
### /trade-idea-generator [optional --send-telegram]

**Purpose:** Generate daily trade ideas from the current portfolio and watchlist, including covered calls, cash-secured puts, defined-risk put spreads, and management alerts for existing short-premium exposure. Writes `outputs/trade-ideas-{DATE}.md` and can send a concise summary to Telegram.

Runs `scripts/trade_idea_generator.py`; uses `context/portfolio-details.md`, `context/watchlist.md`, `FMP_API_KEY` when available, and `TELEGRAM_BOT_TOKEN` plus `TELEGRAM_CHAT_ID` for delivery.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and optionally send Telegram summary | `python3 scripts/trade_idea_generator.py [--send-telegram --telegram-chat-id "$TELEGRAM_CHAT_ID"]` |
