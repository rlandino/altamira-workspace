...
**Purpose:** Generate daily trade ideas from the current repository portfolio and watchlist, write `outputs/trade-idea-generator-{DATE}.md`, and optionally send the Telegram summary.

Runs `python3 scripts/trade_idea_generator.py --send-telegram`. Uses `context/portfolio-details.md` and `context/watchlist.md`, live quotes/VIX/earnings, and options-chain data when available. Prioritizes existing short-premium risk management, covered calls on large holdings, CSPs on top watchlist names, and concentration trims.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and optional Telegram summary | `python3 scripts/trade_idea_generator.py --send-telegram` |
