...
### /trade-idea-generator [telegram]

**Purpose:** Generate a concise portfolio/watchlist trade-idea report and optionally send it to Telegram.

Reads `context/portfolio-details.md` and `context/watchlist.md`, flags open short-premium profit/stop actions, covered-call candidates on large holdings, and B- or better watchlist setups. Writes `outputs/trade-idea-generator-{DATE}.md`; with `telegram`/`send`, uses `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` or an explicit chat id.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and optionally send to Telegram | `python3 scripts/trade_idea_generator.py [--send-telegram --telegram-chat-id CHAT_ID]` |
