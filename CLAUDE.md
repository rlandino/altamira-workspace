...
**Purpose:** Generate the daily market recap, write `outputs/daily-market-recap-{DATE}.md`, and send both a concise summary and the Markdown report to Telegram.

Runs `python3 scripts/daily_market_recap.py --send-telegram`. Requires `FMP_API_KEY`, `TELEGRAM_BOT_TOKEN`, and `TELEGRAM_CHAT_ID` in the environment, or pass `--telegram-chat-id` when needed. Use `--dry-run` to generate and review the Markdown without sending Telegram messages.
...
|--------|---------|-------|
| `scripts/daily_market_recap.py` | Generate a daily market recap Markdown file and optionally send summary + document to Telegram | `FMP_API_KEY=... TELEGRAM_BOT_TOKEN=... TELEGRAM_CHAT_ID=... python3 scripts/daily_market_recap.py --send-telegram` |
