...
**Purpose:** Generate trade ideas from the existing portfolio and watchlist, save a report, and optionally send the Telegram-ready summary to the configured Telegram channel.

Runs `scripts/trade_idea_generator.py`, which reads `context/portfolio-details.md` and `context/watchlist.md`, pulls FMP quotes/earnings data plus Massive.com/FMP options data, ranks CSP, covered-call, and watchlist entry-review ideas, and writes `outputs/trade-idea-generator-{DATE}.md`. Use `--send-telegram` to deliver the summary.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist CSP, covered-call, and entry-review ideas; optionally send Telegram summary | `python3 scripts/trade_idea_generator.py` or `python3 scripts/trade_idea_generator.py --send-telegram` |
