...
**Purpose:** Generate a portfolio-aware daily trade idea from the repository's current portfolio and watchlist context, then send the Telegram-ready summary to the configured channel.

Runs `python3 scripts/trade_idea_generator.py --send-telegram`. Reads `context/portfolio-details.md` and `context/watchlist.md`, skips expired option rows, writes `outputs/trade-idea-generator-{DATE}.md`, `outputs/trade-idea-generator-telegram-{DATE}.txt`, and `outputs/trade-idea-generator-status-{DATE}.json`. Uses `TELEGRAM_BOT_TOKEN` plus `TELEGRAM_CHAT_ID` or the existing Telegram chat ID fallback in `outputs/csp-daily-scan-fixed.json`.
...
| `scripts/trade_idea_generator.py` | Generate a risk-aware trade idea from current portfolio/watchlist context and optionally send Telegram | `python3 scripts/trade_idea_generator.py --send-telegram` |
