...
│   │   ├── trade-idea-generator.md # /trade-idea-generator — portfolio/watchlist trade digest with Telegram delivery
...
**Purpose:** Generate a concise trade-idea digest from the current repository portfolio and watchlist, write the report to `outputs/`, and send the Telegram-ready summary to the configured channel.

Runs `python3 scripts/trade_idea_generator.py --send-telegram`; uses `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`. Requires `TELEGRAM_BOT_TOKEN`; `TELEGRAM_CHAT_ID` is optional because the script can fall back to the channel ID stored in `outputs/csp-daily-scan-fixed.json`.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade-idea reports and optional Telegram digest | `python3 scripts/trade_idea_generator.py --send-telegram` |
