...
│   │   ├── trade-idea-generator.md # /trade-idea-generator — daily portfolio/watchlist trade idea + Telegram summary
...
**Purpose:** Generate one concise, portfolio-aware trade idea from current repository context and optionally send it to Telegram.

Runs `scripts/trade_idea_generator.py`, reading `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`. Writes `outputs/trade-idea-generator-{DATE}.md`; use `--send-telegram` with `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` (or `--chat-id`) to deliver the summary.
...
| `scripts/trade_idea_generator.py` | Generate a daily portfolio/watchlist trade idea and optional Telegram summary | `python3 scripts/trade_idea_generator.py --send-telegram` |
