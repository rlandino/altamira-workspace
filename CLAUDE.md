...
**Purpose:** Generate internal daily trade ideas from `context/portfolio-details.md` and `context/watchlist.md`, write `outputs/trade-idea-generator-{DATE}.md`, and send the concise summary to Telegram when `TELEGRAM_BOT_TOKEN` and a chat/channel id are configured. Optional live quote/VIX refresh uses `FMP_API_KEY`.

Runs `python3 scripts/trade_idea_generator.py`. Use `--limit N`, `--no-telegram`, `--telegram-required`, or `--telegram-chat-id CHAT_ID_OR_CHANNEL` as needed.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas, write report, and send Telegram summary | `python3 scripts/trade_idea_generator.py` (set `TELEGRAM_BOT_TOKEN` and a chat/channel env var for delivery) |
