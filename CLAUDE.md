...
│   │   ├── daily-market-recap.md # /daily-market-recap — generate recap and send summary + markdown to Telegram
...
**Purpose:** Generate a daily market recap and send both the concise summary and markdown report to Telegram.

Runs `scripts/daily_market_recap.py --send-telegram`, using FMP for indices, ETFs, VIX, sectors, movers, earnings, headlines, and historical S&P 500 levels. Writes `outputs/daily-market-recap-{DATE}.md` and optionally `outputs/briefing-chart-{DATE}.png`; requires `FMP_API_KEY` and `TELEGRAM_BOT_TOKEN` plus `TELEGRAM_CHAT_ID`/`TELEGRAM_CHANNEL_ID` or resolvable bot updates.
