...
**Purpose:** Generate daily trade ideas from the current portfolio and watchlist, write an audit report, and optionally send the summary to Telegram.

Runs `scripts/trade_idea_generator.py` against `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`. It fetches market data when available, ranks cash-secured put / put-spread candidates, flags covered-call or trim candidates, reviews active short-premium positions, writes `outputs/trade-idea-generator-{DATE}.md` and `.json`, and can send with `--send-telegram`.
