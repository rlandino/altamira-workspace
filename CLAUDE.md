...
**Purpose:** Generate a daily market recap markdown file and send both the concise summary and markdown document to Telegram.

Runs `scripts/daily_market_recap.py` using FMP quotes, movers, sector snapshot, historical S&P 500 data, headlines, and earnings calendar. Writes `outputs/daily-market-recap-{DATE}.md`; use `--send-telegram` to deliver via `TELEGRAM_BOT_TOKEN` and the configured Telegram chat/channel.
...
| `scripts/daily_market_recap.py` | Generate a daily market recap markdown file and optionally send summary + document to Telegram | `python3 scripts/daily_market_recap.py --send-telegram` |
