...
│   │   ├── daily-market-recap.md # /daily-market-recap — daily recap with Telegram summary + markdown
...
**Purpose:** Generate a daily market recap, write `outputs/daily-market-recap-{DATE}.md`, and send both a short summary and Markdown document to Telegram.

Runs `python3 scripts/daily_market_recap.py --send-telegram`. Uses FMP for indices, VIX, sector leadership, gainers/losers, headlines, and earnings. Requires `TELEGRAM_BOT_TOKEN` or `TELEGRAM_API_TOKEN` plus `TELEGRAM_CHAT_ID` unless `--telegram-chat-id` is passed.
