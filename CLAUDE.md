...
**Purpose:** Generate a daily market recap markdown file and send both a short summary and the markdown document to Telegram.

Runs `scripts/daily_market_recap.py`, fetches FMP market data/headlines/earnings, writes `outputs/daily-market-recap-{DATE}.md` and `outputs/daily-market-recap-summary-{DATE}.txt`, then sends the summary and markdown file through the Telegram Bot API. Requires `FMP_API_KEY`, `TELEGRAM_BOT_TOKEN`, and a Telegram destination via `TELEGRAM_CHAT_ID`, `TELEGRAM_CHANNEL_ID`, `TELEGRAM_CHANNEL`, or `--telegram-chat-id`.
...
| `scripts/daily_market_recap.py` | Generate daily market recap markdown and send summary + file to Telegram | `python3 scripts/daily_market_recap.py --date YYYY-MM-DD` |
