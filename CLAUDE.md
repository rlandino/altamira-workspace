...
**Purpose:** Generate daily trade ideas from the current portfolio and watchlist, write a full report plus Telegram-sized summary, and optionally send it to Telegram.

Runs `python3 scripts/trade_idea_generator.py --send-telegram`. Uses `context/portfolio-details.md` and `context/watchlist.md`, with live FMP quotes/earnings and Massive options data when credentials are available. Writes `outputs/trade-idea-generator-{DATE}.md`, `outputs/trade-idea-generator-telegram-{DATE}.txt`, and a Telegram status file when sending.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and optionally send a Telegram summary | `python3 scripts/trade_idea_generator.py --send-telegram` |
