...
**Purpose:** Generate current trade ideas from the repository's portfolio and watchlist context and optionally send a concise summary to Telegram.

Parses `context/portfolio-details.md` and `context/watchlist.md`, fetches quotes/VIX/earnings/options chains, ranks cash-secured put and covered-call candidates, flags stale or actionable existing short-premium positions, and writes `outputs/trade-idea-generator-{DATE}.md` plus JSON. Use `python scripts/trade_idea_generator.py --send-telegram` with `FMP_API_KEY`, optional `MASSIVE_API_KEY`, and Telegram credentials.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas, write reports, and optionally send Telegram summary | `FMP_API_KEY=... python scripts/trade_idea_generator.py --send-telegram` |
