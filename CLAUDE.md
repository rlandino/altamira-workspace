...
**Purpose:** Generate a daily trade idea digest from the current portfolio, watchlist, and open short-premium context, then optionally send it to Telegram.

Runs `scripts/trade_idea_generator.py`, reads `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`, writes `outputs/trade-idea-generator-{DATE}.md`, and sends the top ideas when Telegram credentials are configured.
...
| `scripts/trade_idea_generator.py` | Generate current portfolio/watchlist trade ideas and optionally send Telegram digest | `python scripts/trade_idea_generator.py --send-telegram` |
