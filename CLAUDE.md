...
│   │   ├── daily-market-recap.md # /daily-market-recap — daily markdown recap + Telegram delivery
...
**Purpose:** Generate a daily markdown market recap and send the summary plus markdown document to Telegram.

Runs `scripts/daily_market_recap.py`; uses FMP quotes, sectors, movers, headlines, earnings, and historical prices. Writes `outputs/daily-market-recap-{DATE}.md`; with `--send-telegram`, sends the short recap and markdown file via Telegram Bot API.
...
|--------|---------|-------|
| `scripts/daily_market_recap.py` | Generate `outputs/daily-market-recap-{DATE}.md` and optionally send summary + markdown to Telegram | `python3 scripts/daily_market_recap.py --send-telegram` (set `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID`, or pass `--telegram-chat-id`) |
