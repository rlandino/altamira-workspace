...
│   │   ├── trade-idea-generator.md # /trade-idea-generator — portfolio/watchlist CSP ideas + Telegram
...
**Purpose:** Generate daily cash-secured put trade ideas from the current repository portfolio and watchlist, write a report, and send the summary to Telegram.

Reads `context/portfolio-details.md` and `context/watchlist.md`, fetches live quotes, VIX, upcoming earnings, and option chains via FMP/Massive, excludes contracts with earnings conflicts, writes `outputs/trade-idea-generator-{DATE}.md`, and sends the concise summary with `python3 scripts/trade_idea_generator.py --send-telegram`.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist CSP trade ideas, write a report, and optionally send Telegram | `python3 scripts/trade_idea_generator.py --send-telegram` |
