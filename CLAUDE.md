...
**Purpose:** Daily market recap with markdown output and Telegram delivery.

Runs `python3 scripts/daily_market_recap.py --send-telegram` to fetch FMP market data, write `outputs/daily-market-recap-{DATE}.md`, send a concise summary message to Telegram, and attach the markdown file. Uses `TELEGRAM_BOT_TOKEN`/`TELEGRAM_API_TOKEN` and `TELEGRAM_CHAT_ID` (or `--telegram-chat-id`).
