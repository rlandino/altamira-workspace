...
**Purpose:** Generate a concise trade idea scan from the current repository portfolio, open short premium positions, and watchlist; optionally sends the summary to Telegram.

Runs `scripts/trade_idea_generator.py`, reads `context/portfolio-details.md`, `context/options-positions.md`, and `context/watchlist.md`, and writes `outputs/trade-idea-generator-{DATE}.md`. Use `--send-telegram` for Telegram delivery.
