...
│   │   ├── trade-idea-generator.md # /trade-idea-generator — daily portfolio/watchlist trade ideas + Telegram
...
**Purpose:** Generate daily trade ideas from the existing portfolio and watchlist context, write an audit report and JSON payload, and optionally send the concise summary to Telegram.

Runs `scripts/trade_idea_generator.py` against `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`; fetches live FMP quotes/history; ranks CSP/put-spread and covered-call/trim candidates; filters expired static option rows; and writes `outputs/trade-idea-generator-{DATE}.md` plus `.json`.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas, write report/JSON, and optionally send Telegram summary | `python3 scripts/trade_idea_generator.py --include-options-chain --send-telegram` |
