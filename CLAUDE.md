...
│   │   ├── daily-market-recap.md # /daily-market-recap — daily recap with Telegram summary + Markdown delivery
...
**Purpose:** Generate Altamira Capital's daily market recap, save a Markdown report, and send both a short summary and the Markdown file to the configured Telegram channel.

Run `python3 scripts/daily_market_recap.py --send-telegram` from the workspace root. Uses FMP for indices, VIX, sector leadership, movers, headlines, and earnings; writes `outputs/daily-market-recap-{DATE}.md`. Telegram delivery requires `TELEGRAM_BOT_TOKEN` or `TELEGRAM_API_TOKEN` plus `TELEGRAM_CHAT_ID` (or pass `--telegram-chat-id`).
