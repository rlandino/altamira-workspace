...
**Purpose:** Generate a concise daily market recap markdown report and send both the text summary and markdown file to Telegram.

Runs `scripts/daily_market_recap.py`, pulling FMP quotes, movers, sector snapshot, S&P 500 technical context, earnings calendar, and broad-market headlines. Writes `outputs/daily-market-recap-{DATE}.md`; requires `FMP_API_KEY`, `TELEGRAM_BOT_TOKEN`, and `TELEGRAM_CHAT_ID` (or `--chat-id`) for delivery.
...
|--------|---------|-------|
| `scripts/daily_market_recap.py` | Generate daily market recap markdown and send summary + report to Telegram | `python3 scripts/daily_market_recap.py` (requires `FMP_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` or `--chat-id`) |
