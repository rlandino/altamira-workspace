...
│   │   ├── daily-market-recap.md # /daily-market-recap — daily recap report with Telegram summary/file delivery
...
Runs `python3 scripts/daily_market_recap.py --send-telegram`; uses FMP for indices, VIX, market movers, sector performance, headlines, and upcoming earnings. Writes `outputs/daily-market-recap-{DATE}.md`. Requires `TELEGRAM_BOT_TOKEN` or `TELEGRAM_API_TOKEN` plus `TELEGRAM_CHAT_ID` unless a chat ID is passed explicitly.
...
| `scripts/daily_market_recap.py` | Generate daily market recap markdown and optionally send summary plus document to Telegram | `python3 scripts/daily_market_recap.py --send-telegram` |
