...
**Purpose:** Generate a daily trade idea brief from the current repository portfolio/watchlist/options context and optionally send the concise brief to Telegram.

Runs `python3 scripts/trade_idea_generator.py --send-telegram`, reading `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`. Writes `outputs/trade-idea-generator-{DATE}.md`, `outputs/trade-idea-generator-{DATE}-telegram.txt`, and a Telegram status JSON when sent. Requires `TELEGRAM_BOT_TOKEN`; uses `TELEGRAM_CHAT_ID` or the existing CSP workflow chat fallback.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and send the concise brief to Telegram | `python3 scripts/trade_idea_generator.py --send-telegram` |
