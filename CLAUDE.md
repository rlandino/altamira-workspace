...
**Purpose:** End-of-day market recap with Telegram delivery. Generates `outputs/daily-market-recap-{DATE}.md` and `outputs/daily-market-recap-{DATE}.summary.json`, then sends both a concise HTML summary and the markdown file to Telegram when `--send-telegram` is used.

Runs `python3 scripts/daily_market_recap.py --send-telegram`. Uses FMP for indices, VIX, sectors, gainers/losers, earnings, historical S&P 500 context, and broad market headlines. Requires `TELEGRAM_BOT_TOKEN`; uses `TELEGRAM_CHAT_ID` when set or the workspace Telegram fallback.
...
| `scripts/daily_market_recap.py` | Generate end-of-day market recap markdown and optionally send summary plus file to Telegram | `python3 scripts/daily_market_recap.py --send-telegram` |
