...
**Purpose:** Generate a concise trade-idea digest from the current repository portfolio and watchlist, write it to `outputs/`, and send it to Telegram.

Runs `python3 scripts/trade_idea_generator.py --send-telegram`, parsing `context/portfolio-details.md`, `context/watchlist.md`, and `context/options-positions.md`. It highlights concentration-management ideas, new-money watchlist candidates, expired-option housekeeping, and avoid/low-priority names.
...
|--------|---------|-------|
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and optionally send the digest to Telegram | `python3 scripts/trade_idea_generator.py --send-telegram` |
