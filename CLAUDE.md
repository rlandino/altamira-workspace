...
**Purpose:** Generate daily trade ideas from the current portfolio and watchlist, write a dated markdown report, and optionally send the concise summary to Telegram.

Runs `python3 scripts/trade_idea_generator.py --send-telegram` by default for delivery. Uses `context/portfolio-details.md`, `context/options-positions.md`, and `context/watchlist.md`; fetches FMP quotes/history/earnings and Massive.com option snapshots when keys are available; writes `outputs/trade-idea-generator-{DATE}.md`.

Example: `/trade-idea-generator` or `python3 scripts/trade_idea_generator.py --no-telegram`
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas, write a dated report, and optionally send Telegram | `python3 scripts/trade_idea_generator.py --send-telegram` |
