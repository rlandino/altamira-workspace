...
**Purpose:** Generate ranked trade ideas from the current repository portfolio and watchlist, write `outputs/trade-idea-generator-{DATE}.md`, and optionally send the summary to Telegram.

Runs `scripts/trade_idea_generator.py`, using `context/portfolio-details.md` and `context/watchlist.md`. For Telegram delivery, set `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` or pass `--telegram-chat-id`.
...
| `scripts/trade_idea_generator.py` | Generate ranked portfolio/watchlist trade ideas and optionally send a Telegram alert | `python scripts/trade_idea_generator.py --send-telegram --telegram-chat-id CHAT_ID` |
