...
**Purpose:** Generate the daily market recap markdown and send both summary text and markdown file to Telegram.

Runs `python3 scripts/daily_market_recap.py`, which fetches FMP index/ETF, gainers/losers, sectors, earnings, and SPX EOD data; computes trend, 5D/20D context, and support/resistance; writes `outputs/daily-market-recap-{DATE}.md`; then sends a concise recap message plus the markdown file via Telegram bot API.
...
| `scripts/daily_market_recap.py` | Build daily market recap markdown and deliver summary + file to Telegram | `python3 scripts/daily_market_recap.py` (optional: `--date YYYY-MM-DD`, `--chat-id <ID>`, `--dry-run`) |
