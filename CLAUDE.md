...
│   │   ├── daily-market-recap.md # /daily-market-recap — daily market recap + Telegram delivery
...
**Purpose:** Generate a daily market recap markdown file and send both a concise summary and the `.md` report to Telegram.

Runs `scripts/daily_market_recap.py --send-telegram`, using FMP for index/ETF/VIX quotes, sector performance, gainers/losers, headlines, S&P 500 trend metrics, watchlist movers, and the next 7 days of earnings. Writes `outputs/daily-market-recap-{DATE}.md`; requires `TELEGRAM_BOT_TOKEN` plus `TELEGRAM_CHAT_ID` / `TELEGRAM_CHANNEL_ID` or `--telegram-chat-id`.
...
| `scripts/daily_market_recap.py` | Generate daily market recap markdown and send summary/report to Telegram | `python scripts/daily_market_recap.py --send-telegram [--telegram-chat-id CHAT_ID]` |
