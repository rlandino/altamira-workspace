...
**Purpose:** Generate the end-of-day Altamira market recap and deliver it to Telegram.

Runs `scripts/daily_market_recap.py --send-telegram`; writes `outputs/daily-market-recap-{DATE}.md`, sends a concise summary via Telegram, and attaches the markdown file. Uses `TELEGRAM_BOT_TOKEN` plus `TELEGRAM_CHAT_ID`/`TELEGRAM_CHANNEL_ID` when set, with the existing workspace Telegram chat fallback.
...
| `scripts/daily_market_recap.py` | Generate the daily market recap markdown and optionally send summary + report to Telegram | `python3 scripts/daily_market_recap.py --send-telegram` |
