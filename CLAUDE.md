...
**Purpose:** Generate daily trade ideas from the current portfolio and watchlist, including CSP/bull put spread candidates, covered-call/trim candidates, and open short-premium management notes. Writes `outputs/trade-idea-generator-{DATE}.md`; with `--send-telegram`, sends the concise summary using `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID`.
...
|--------|---------|-------|
| `scripts/trade_idea_generator.py` | Generate daily portfolio/watchlist trade ideas and optional Telegram summary | `python3 scripts/trade_idea_generator.py --send-telegram` |
