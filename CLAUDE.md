...
**Purpose:** Generate ranked daily trade ideas from the current portfolio and watchlist, write markdown and Telegram summaries, and optionally send the concise summary to Telegram.

Reads `context/portfolio-details.md` and `context/watchlist.md`; fetches quotes, VIX, earnings, and options chains when credentials are available; prioritizes short-premium risk management, covered calls, cash-secured puts, and concentration trims. Writes `outputs/trade-idea-generator-{DATE}.md`, `outputs/trade-idea-generator-telegram-{DATE}.txt`, and `outputs/trade-idea-generator-telegram-status-{DATE}.txt` when sending.
...
| `scripts/trade_idea_generator.py` | Generate daily portfolio/watchlist trade ideas and optionally send the Telegram summary | `python3 scripts/trade_idea_generator.py --send-telegram` |
