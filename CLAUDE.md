...
**Purpose:** Generate the daily market recap markdown file and send both the summary and report document to Telegram.

Runs `scripts/daily_market_recap.py` using FMP quotes, sector data, movers, headlines, earnings calendar, and S&P 500 technicals. Writes `outputs/daily-market-recap-{DATE}.md`; use `--send-telegram` with `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` (or `--telegram-chat-id`) to deliver to Telegram.
