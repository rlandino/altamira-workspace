...
│   │   ├── trade-idea-generator.md # /trade-idea-generator — portfolio/watchlist trade ideas + Telegram send
...
**Purpose:** Generate trade ideas from the current repository portfolio, open short-premium positions, and watchlist. Writes `outputs/trade-idea-generator-{DATE}.md`; with `--send-telegram`, sends the concise summary to Telegram using `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` (or `--chat-id`).

Runs `python3 scripts/trade_idea_generator.py`. The report emphasizes existing options management, concentration risk, staged watchlist candidates, and an execution checklist. Ideas are research prompts only; confirm live prices, earnings dates, liquidity, IV rank, cash, and risk limits before action.
...
| `scripts/trade_idea_generator.py` | Generate portfolio/watchlist trade ideas and optionally send the summary to Telegram | `python3 scripts/trade_idea_generator.py [--send-telegram --chat-id CHAT_ID]` |
