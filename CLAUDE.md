...
| `scripts/jane_street_pre_market_edge_analyzer.py` | Generate the Jane Street pre-market theta briefing and optionally send it to Telegram | `python scripts/jane_street_pre_market_edge_analyzer.py --send-telegram` (uses `TELEGRAM_BOT_TOKEN` and optional `TELEGRAM_CHAT_ID`) |
