...
Runs `python3 scripts/daily_market_recap.py --send-telegram`; uses FMP quotes, gainers/losers, sector snapshot with sector ETF fallback, headlines, earnings calendar, and S&P 500 technicals. Writes `outputs/daily-market-recap-{DATE}.md`. Telegram delivery requires `TELEGRAM_BOT_TOKEN` (or `TELEGRAM_API_TOKEN`) and `TELEGRAM_CHAT_ID`, or pass `--telegram-chat-id`.
