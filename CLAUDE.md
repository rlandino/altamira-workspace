...
**Purpose:** Generate a daily market recap markdown report and send both a plain-text summary and the markdown file to Telegram.

Runs `scripts/daily_market_recap.py`, using FMP for indices, movers, sectors, headlines, earnings, and historical S&P 500 closes. Writes `outputs/daily-market-recap-{DATE}.md`, optionally writes `outputs/daily-market-recap-chart-{DATE}.png`, and sends Telegram delivery when `--send-telegram` is used with `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` / `TELEGRAM_CHANNEL_ID`.
...
|--------|---------|-------|
| `scripts/daily_market_recap.py` | Generate daily market recap markdown, chart, and optional Telegram summary/document delivery | `FMP_API_KEY=... TELEGRAM_BOT_TOKEN=... TELEGRAM_CHAT_ID=... python3 scripts/daily_market_recap.py --send-telegram` |
