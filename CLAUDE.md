...
**Purpose:** Generate Altamira Capital's daily market recap, write a Markdown report, and send both a short summary and the Markdown file to Telegram.

Runs `python3 scripts/daily_market_recap.py --send-telegram`; uses FMP quotes, movers, sector snapshot or sector ETF fallback, headlines, and earnings calendar. Writes `outputs/daily-market-recap-{DATE}.md`.
