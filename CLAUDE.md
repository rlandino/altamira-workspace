...
**Purpose:** Generate daily trade ideas from current repository portfolio, open options, and watchlist context, then optionally send the summary to Telegram.

Runs `scripts/trade_idea_generator.py`, reads `context/portfolio-details.md`, `context/options-positions.md`, and `context/watchlist.md`, writes `outputs/trade-idea-generator-{DATE}.md`, and sends Telegram when run with `--send-telegram` using `TELEGRAM_BOT_TOKEN` plus `TELEGRAM_CHAT_ID` or an existing workflow `chatId`.
